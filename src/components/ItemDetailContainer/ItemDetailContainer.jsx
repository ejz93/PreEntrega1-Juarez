...
const ItemDetailContainer = () => {
  ...
  return (
    <Fragment>
      <button className="btn btn-dark border border-warning m-2 p-2" onClick={() => navigate(-1)}>
        Back
      </button>
      <hr />
  ...
  <div className="text-warning spinner-border ms-left" role="status" aria-hidden="true"></div>
      ...
      {item && !loading && (
        <div className="container-fluid">
            ...
            <hr />
            <ItemCount />
            <button className="btn btn-warning border border-dark m-2 p-2 fs-5" onClick={addToCart}>Add to cart</button>
          </div>
      )}
    </Fragment>
  );
